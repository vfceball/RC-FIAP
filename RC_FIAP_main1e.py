...
global Loc_span, Loc_heigth, ListNodes, Elements, DataBeamDesing, DataColDesing, Wtotal, num_elems, ListNodesDrift, \
    cIndex, ListNodesBasal, T1m, Wtotal, IM, Sa_max, RDR_max, SDR_max, nrecs
...
class RegistroColumns:
    def __init__(self, tbl_data_design_columns, id_, b, h, ro, db, nbH, nbB, Leg_n_H, Leg_n_B, Sstirrup):
        ...
        tbl_data_design_columns.setCellWidget(fila, 5, self.spx_nbH)
        tbl_data_design_columns.setCellWidget(fila, 6, self.spx_nbB)
        tbl_data_design_columns.setCellWidget(fila, 7, self.spx_Leg_n_H)
        tbl_data_design_columns.setCellWidget(fila, 8, self.spx_Leg_n_B)
        tbl_data_design_columns.setCellWidget(fila, 9, self.spx_Sstirrup)
    ...
    def Pushover(self):
        ...
        def singlePush1(dref, mu, ctrlNode, dispDir, nSteps):
            ...
            return maxDriftPiso, maxDriftTecho, maxDriftTecho2, VBasal_v, MaxDriftStory_v, DriftTecho_v, \
                   MP_ElemsForceS1, MP_ElemsDeforS1, MP_ElemsForceS6, MP_ElemsDeforS6, DriftStory_m
        ...
        if self.ui.radioButtonFast.isChecked():
            ...
            MP_ElemsDeforS1, MP_ElemsForceS6, MP_ElemsDeforS6, DriftStory_m = singlePush1(dref, mu, IDctrlNode,
                                                                                          IDctrlDOF, nSteps)
        ...
        for jj in range(nSDR):
            RDR = np.interp(AlphaCurves[jj], MaxDriftStory_v[:], DriftTecho_v[:])
            for ii in range(floors_num - 1):
                SDR = np.interp(AlphaCurves[jj], MaxDriftStory_v[:], DriftStory_m[1:, ii])
                Alpha[jj, ii] = SDR / RDR
